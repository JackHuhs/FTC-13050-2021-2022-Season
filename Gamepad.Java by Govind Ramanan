...
public class Gamepad extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        firstWheel = hardwareMap.get(DcMotor.class, "firstWheel");
        ...
        while (opModeIsActive()) {
            ...
            double intakePower;
            launcherPower = 0.85; //different speeds, change to 1.00 for 100 percent of the motor's RPM
            intakePower = .5; //change later
            //launcher
            //
            if(gamepad1.right_trigger>0.5){wobbleArm.setPower(launcherPower);}else if(gamepad1.right_bumper){wobbleArm.setPower(-launcherPower);}else{wobbleArm.setPower(0);}
            if(gamepad1.left_trigger>0.5){firstWheel.setPower(launcherPower);}else if(gamepad1.left_bumper) {firstWheel.setPower(-launcherPower);}else{firstWheel.setPower(0);}
            
            //Turning
            if(gamepad1.left_stick_x>0.75){
                leftDrive.setPower(launcherPower);
                rightDrive.setPower(-launcherPower);
                leftDownDrive.setPower(launcherPower);
                rightDownDrive.setPower(-launcherPower);
